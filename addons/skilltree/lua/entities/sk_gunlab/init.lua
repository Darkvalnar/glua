...
function ENT:Use(activator)
	...
	// AWARD EXP AND COOLDOWN  START
	if ( SK_TREE.EXP_GAIN.GUNLAB.ENABLED == true ) then 
		local canrewardexp = activator:SK_ENT_COOLDOWN_CHECK( self )
		
		if ( canrewardexp == true ) then
			activator:SK_ENT_COOLDOWN_SET( SK_TREE.EXP_GAIN.GUNLAB.COOLDOWN, self, SK_TREE.EXP_GAIN.GUNLAB.BUY_AMOUNT  )
		end
	end
	// AWARD EXP AND COOLDOWN  END
	...
	if IsValid(owner) and activator ~= owner then
		...
		// AWARD EXP AND COOLDOWN  START
		if ( SK_TREE.EXP_GAIN.GUNLAB.ENABLED == true ) then 
			local canrewardexp = owner:SK_ENT_COOLDOWN_CHECK( self )
			
			if ( canrewardexp == true ) then
				owner:SK_ENT_COOLDOWN_SET( SK_TREE.EXP_GAIN.GUNLAB.COOLDOWN, self, SK_TREE.EXP_GAIN.GUNLAB.SELL_AMOUNT  )
			end
		end
		// AWARD EXP AND COOLDOWN  END
	end
