...
function ENT:Initialize()
	...
	self.sparking = false
	self.damage = 100 * ( 1 + hook.Call( "SK_TREE_MUL", GAMEMODE, self:Getowning_ent(), "printer_upgrade_armour" ) )
	self.IsMoneyPrinter = true
	
	local mul = 1 - hook.Call( "SK_TREE_MUL", GAMEMODE, self:Getowning_ent(), "printer_lessons" )

	timer.Simple(math.random(100 * mul, 350 * mul), function() PrintMore(self) end)
...
function ENT:CreateMoneybag()
	...
	local mul = hook.Call( "SK_TREE_MUL", GAMEMODE, self:Getowning_ent(), "printer_upgrade_extra" )
	
	amount = amount + amount * mul
	...
	// EDIT: Add exp to player
	if ( SK_TREE.EXP_GAIN.PRINTERS.ENABLED == true ) then
		if ( SK_TREE.EXP_GAIN.PRINTERS.TYPES[self:GetClass()] ) then self:Getowning_ent():SK_AddExp( SK_TREE.EXP_GAIN.PRINTERS.TYPES[self:GetClass()], "Gained %s for printing money" ) end
	end
	
	mul = 1 + hook.Call( "SK_TREE_MUL", GAMEMODE, self:Getowning_ent(), "printer_lessons" )
