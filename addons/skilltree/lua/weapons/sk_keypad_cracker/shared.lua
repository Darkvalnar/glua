...
function SWEP:calcKeypadTime()
	if ( !self.Owner or !self.Owner:IsValid() ) then return end
	
	local time = keypad_crack_time:GetInt()
	time = time - time * hook.Call( "SK_TREE_MUL", GAMEMODE, self.Owner, "basic_electronics" )
...
function SWEP:Initialize()
	...
	self.KeyCrackTime = self:calcKeypadTime()
end	
		...
		if SERVER then
			...
			timer.Create("KeyCrackSounds: "..self:EntIndex(), 1, self.KeyCrackTime, function()
				if IsValid(self) and self.IsCracking then
					self:EmitSound( self.KeyCrackSound, 100 * ( 1 - hook.Call( "SK_TREE_MUL", GAMEMODE, self.Owner, "basic_electronics" ) ), 100 )	
				end
			end)
		else
